...
def generate_secret_code(length=6):
    """Generates a random 6-digit secret code and its hash using SHA-256 for speed."""
    # Ensure the code is 6 digits long
    raw_code = ''.join(secrets.choice(string.digits) for _ in range(length))
    
    # ⭐ HASHING FIX 1: Use a fast cryptographic hash (SHA-256) for speed.
    # We use FLASK_SECRET_KEY as an application-wide salt for all codes.
    salt = os.getenv("FLASK_SECRET_KEY", "default-salt").encode('utf-8')
    data_to_hash = (salt + raw_code.encode('utf-8'))
    
    # Prepend 'sha256$' for easy identification in the database
    hashed_code = 'sha256$' + hashlib.sha256(data_to_hash).hexdigest()
...
# ... (generate_households_from_recipe and get_voting_status remain the same) ...
...
# --- NEW HELPER FOR CHECKING SHA-256 CODES (Required) ---
def check_sha256_code(raw_code, stored_hash):
    """Checks a raw code against a SHA-256 hash generated by generate_secret_code."""
    if not stored_hash or not isinstance(stored_hash, str) or not stored_hash.startswith('sha256$'):
        return False
        
    _, stored_hex = stored_hash.split('$', 1)
    
    salt = os.getenv("FLASK_SECRET_KEY", "default-salt").encode('utf-8')
    data_to_hash = (salt + raw_code.encode('utf-8'))
    
    current_hex = hashlib.sha256(data_to_hash).hexdigest()
        ...
        try:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                for row in admin_rows:
                    ...
                    # ⭐ HASHING CHANGE 2: Check password hash (uses slower PBKDF2/Argon2 check)
...
def home_management(): 
    ...
    if request.method == 'POST':
        ...
        try:
            # --- Type determination logic (UNCHANGED) ---
            ...
            # 2. Get existing households (NOT NEEDED FOR CODE REGEN, but needed for deletion logic)
            existing_houses = {}
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute(
                    "SELECT tower, flat FROM households WHERE society_name = %s", 
                    (society_name,)
                )
                # Store existing houses as a key for fast deletion lookup: (tower, flat)
                existing_houses = {
                    (h['tower'], h['flat']): True 
                    for h in cur.fetchall()
                }

            # 3. Prepare the list of values for UPSERT with *NEW* secret codes AND prepare the report
            insert_values = []
            ...
            for society_name_item, tower, flat in final_household_list:
                ...
                # ⭐ CRITICAL CHANGE: Always generate a new code/hash
                ...
                insert_values.append((society_name_item, tower, flat, hashed_code_for_db, '')) 
            
            # 4. Execute the UPSERT: Insert new houses and UPDATE secret codes on conflicts
            with conn.cursor() as cur:
                if insert_values:
                    psycopg2.extras.execute_values(
                        ...
                        VALUES %s
                        ON CONFLICT (society_name, tower, flat) 
                        DO UPDATE SET secret_code = EXCLUDED.secret_code, mobile_number = EXCLUDED.mobile_number 
                        """, 
                        insert_values,
                        template="(%s, %s, %s, %s, %s)",
                        page_size=1000
                ...
                # 5. Delete removed households (UNCHANGED)
                ...
                # 6. Save the Recipe JSON (UNCHANGED)
                cur.execute(
                    """
            ...
            # 7. CRITICAL FIX: Final Steps: Check results and respond
...
# The updated route for uploading and UPSERTING secret codes
...
def upload_secret_codes():
    ...
    if not file or not ('.' in file.filename and file.filename.rsplit('.', 1)[1].lower() in ['xlsx', 'xls']):
        flash("Invalid or missing Excel file. Please upload an .xlsx or .xls file.", "danger")
        return redirect(url_for("admin_panel"))
    ...
    try:
        ...
        # Determine the housing type for logic branching
        with conn.cursor() as cur:
            cur.execute("SELECT housing_type FROM admins WHERE society_name = %s", (society_name,))
            row = cur.fetchone()
            housing_type = row[0] if row else ''
            
        # Standardize society name for database use
        society_name_upper = society_name.strip().upper()
        ...
        for _, r in df.iterrows():
            ...
            # ⭐ HASHING FIX: Use the fast SHA-256 hashing mechanism
            hashed_code = hash_secret_code(secret_code)

            if housing_type.startswith("Apartment") or housing_type.startswith("Villas-Lanes"):
                # Requires tower, flat, society_name for uniqueness
                if tower:
                    data_to_upsert.append((society_name_upper, tower, flat, hashed_code))
            
            elif housing_type.startswith(("Villas-No Lanes", "Civil")):
                # Requires flat, society_name for uniqueness
                # Tower is set to a standard placeholder like 'NA' in the database if not provided
